...
function getHtmlReport(report: SecurityReport): string {
    ...
    const vulnCards = [...vulnerabilities]
      ...
      <div class="vuln-card" style="border-left: 3px solid ${c}; background: ${bg}">
        ...
        ${vuln.location.snippet ? `<pre class="vuln-snippet"><code>${escapeHtml(vuln.location.snippet)}</code></pre>` : ''}
      </div>`;
        }).join('');
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .vuln-snippet { background: #010409; border: 1px solid var(--border); border-radius: 6px; padding: 10px 12px; margin-top: 10px; font-size: 11px; font-family: var(--mono); overflow-x: auto; white-space: pre; line-height: 1.5; }
